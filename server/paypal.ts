...
// Only initialize PayPal if credentials are provided
if (PAYPAL_CLIENT_ID && PAYPAL_CLIENT_SECRET) {
  ...
  console.warn("PayPal credentials not provided. PayPal payments will not work.");
}
...
export async function createPaypalOrder(req: Request, res: Response) {
  if (!ordersController) {
    return res.status(503).json({ error: "PayPal not configured. Please add PAYPAL_CLIENT_ID and PAYPAL_CLIENT_SECRET." });
  }
